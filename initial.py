...
#适用于60CM望远镜数据的地址生成函数（CCD版）
def address_cope_60old (band_info,target_name):
...
#适用于60CM望远镜数据的地址生成函数(CMOS版)
def address_cope_60new (band_info,target_name):
    ...
    star_cat_file_path = []
    #用于测试增益的本底地址
    biasfile1 = raw_folder + 'bias-0001.fit'
    biasfile2 = raw_folder + 'bias-0002.fit'
    #用于测试增益的平场地址
    flatfile1 = []
    flatfile2 = []
    #参考星所在目录
    ...
    for i in range(len(band_info)):
        raw_flat.append(raw_folder + 'flat-*' + band_info[i] + '.fit')
        super_flat.append(reduced_folder + 'superflat_' + band_info[i] + '.fits')
        raw_data_file_path.append(raw_folder+target_name+'-*'+band_info[i]+'.fit')
        cor_data_file_path.append(reduced_folder+'Cor_'+target_name+'-*'+band_info[i]+'.fits')
        cut_data_file_path.append(reduced_folder+'Cut_Cor_'+target_name+'-???'+band_info[i]+'.fits')
        ast_data_file_path.append(reduced_folder+'Cut_Cor_'+target_name+'-???'+band_info[i]+'-ast.fits')
        star_cat_file_path.append(reduced_folder+'Cut_Cor_'+target_name+'-*'+band_info[i]+'-ast-cat.fits')
        flatfile1.append(raw_folder+'flat-0001'+band_info[i]+'.fit')
        flatfile2.append(raw_folder+'flat-0002'+band_info[i]+'.fit')
        confirmed_star_candidate_path.append(confirmed_star_path + band_info[i] + '_band_confirmed_star_*.fits')
        target_star_path.append(confirmed_star_path + band_info[i] + '_band_target_star.fits')
...
#适用于80CM望远镜的地址生成函数
def address_cope_80 (band_info,target_name):
...
#适用于85CM望远镜的地址生成函数
def address_cope_60 (band_info,target_name,date_obs):
    ...
    #原始本底地址
    raw_bias = raw_folder +'/'+date_obs+'bias-*.fit'
    #超级本底地址
    super_bias = reduced_folder + 'superbias.fits'
    #改正后数据存储位置
    reduced_data_file_path = reduced_folder
    ...
    star_cat_file_path = []
    #用于测试增益的本底地址
    biasfile1 = raw_folder + date_obs +'bias-0001.fit'
    biasfile2 = raw_folder + date_obs +'bias-0002.fit'
    #用于测试增益的平场地址
    flatfile1 = []
    flatfile2 = []
    #参考星所在目录
    ...
    for i in range(len(band_info)):
        raw_flat.append(raw_folder + date_obs +'flat-*' + band_info[i] + '.fit')
        super_flat.append(reduced_folder + 'superflat_' + band_info[i] + '.fits')
        raw_data_file_path.append(raw_folder+date_obs+target_name+'-*'+band_info[i]+'.fit')
        cor_data_file_path.append(reduced_folder+'Cor_'+date_obs+target_name+'-*'+band_info[i]+'.fits')
        cut_data_file_path.append(reduced_folder+'Cut_Cor_'+date_obs+target_name+'-*'+band_info[i]+'.fits')
        ast_data_file_path.append(reduced_folder+'Cut_Cor_'+date_obs+target_name+'-????'+band_info[i]+'-ast.fits')
        star_cat_file_path.append(reduced_folder+'Cut_Cor_'+date_obs+target_name+'-*'+band_info[i]+'-ast-cat.fits')
        flatfile1.append(raw_folder+ date_obs +'flat-0001' + band_info[i]+'.fit')
        flatfile2.append(raw_folder+ date_obs +'flat-0002' + band_info[i]+'.fit')
        confirmed_star_candidate_path.append(confirmed_star_path + band_info[i] + '_band_confirmed_star_*.fits')
        target_star_path.append(confirmed_star_path + band_info[i] + '_band_target_star.fits')
